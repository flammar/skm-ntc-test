...
public class main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Встретив в задании слово "цифры", я понял, что речь идйт от символьном представлении числа, а не о байтовом
        // как в java.io.DataInputStream и java.io.DataOutputStream, К тому же я понял, что числа надо разделять, чтобы
        // "хвост" одного числа не слипся с началом другого, порождая фантомные данные.
        // В случае бинарного формата для этого можно было бы исопльзовать тот факт, что длина байтового представления
        // числа формата int всегда постоянна и равна 4. 

        Thread odds = new Thread(newWriter(toOdd));
        Thread evens = new Thread(newWriter(toEven));
    ...
    private static Runnable newWriter(IntUnaryOperator intUnaryOperator) {
        return () -> {
...
//                    ps.println нельзя использовать так как он делает две хзаписи в файл, числа могут слепляться
                    ps.print(intUnaryOperator.applyAsInt(random.nextInt()) + "\n");
...
//                    sleep(200);
    ...
    private static Runnable newReader() {
        ...
        return () -> {
            // Дождаться пока файл начнёт существовать, иначе Reader упадёт с ошибкой, что файл не найден.
            while (!new File(filename).exists())
                // промежуток времени выбран произвольно
                sleep(100);
            try (FileInputStream is = new FileInputStream(filename)) {
                try {
                    // Самое простое: всё что записано в файл, переписывать в консоль через буфер произвольного размера 
                    while (true) {
    ...
    static long trySleepTill(long ms1) {
        try {
            // Может, хотя и не должен по спецификации, проснуться раньше времени, надо это контролировать
